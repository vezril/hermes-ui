...
import { emptyMetrics, type HermesMetrics } from "./metrics";
import type {
  Labels,
  PublishInput,
  PublishResult,
...
export function fixtureHermesClient(): HermesClient {
  return {
    ...
    async getMetrics(): Promise<HermesMetrics> {
      const m = emptyMetrics();
      // Fixed, illustrative producer counts so the offline UI shows a plausible
      // "who produces" story (HermesMQ has no REST producer listing).
      ...
      for (const [topicId, t] of store.entries()) {
        if (t.deleted) continue;
        m.publishedByTopic[topicId] = t.publishedTotal;
        m.producersByTopic[topicId] = producers[topicId] ?? 0;
      }
      // A couple of topics dedup a small share of publishes.
      m.dedupByTopic["orders.events"] = 214;
      for (const s of subscriptions.values()) {
        m.backlogBySub[s.subscriptionId] = s.backlog;
        m.oldestUnackedBySub[s.subscriptionId] = s.oldestUnackedAgeSeconds;
        m.redeliveredBySub[s.subscriptionId] = s.redeliveredTotal;
        m.deadLetteredBySub[s.subscriptionId] = s.deadLetteredTotal;
        // One active consumer per subscription except the idle archiver.
        m.consumersBySub[s.subscriptionId] = s.subscriptionId === "audit.archiver" ? 0 : 1;
      }
      return m;
    },
  };
